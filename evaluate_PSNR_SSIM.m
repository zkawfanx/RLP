%%% PSNR and SSIM evaluation script from MPRNet project
%%% https://github.com/swz30/MPRNet/tree/main/Deraining

clc;close all;clear all;

methods = {'UNet', 'UNet_RLP', 'UNet_RLP_RPIM', 'Uformer_T', 'Uformer_T_RLP', 'Uformer_T_RLP_RPIM'};
num_set = length(methods);
...
for idx_set = 1:num_set
    file_path = strcat('/path/to/results/', methods{idx_set}, '/');
    %gt_path = strcat('d:/Videos/test_result_rlp/',  'gt/');
    ...
    if img_num > 0 
        parfor j = 1:img_num 
           ...
           gt = imread(strcat(gt_path, image_name(1:4), '.png'));
    ...
    fprintf('For method %s PSNR: %f SSIM: %f\n', methods{idx_set}, qm_psnr, qm_ssim);
